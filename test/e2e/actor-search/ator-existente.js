...
test('Ao buscar um ator existente', function (assert) {
  assert.plan(8);
  
  nightmare
    .goto('http://localhost:3000')
    .type('header.navbar input', 'Robert De Niro')
    .select('header.navbar select', 'actor')
    ...
    .then(function(data){
      ...
      assert.equal(data.href, 'http://localhost:3000/#/actor/Robert De Niro', 'Deve estar na url esperada.');
